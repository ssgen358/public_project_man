...
> [!NOTE]
> **ステークホルダーの位置はプロジェクト中に変わる。** 初期は低関心だった部門が、開発後半にシステム影響を受けると突然高影響力・高関心度に変化することがある。登録簿は最低でもフェーズ移行時に見直すこと。
...
> [!WARNING]
> **「あと少しで解決できそう」という判断での報告先送りが最もリスクが高い。** 課題やリスクの情報を「まだ自分で対処できる」「もう少ししたら解決する」という理由で遅らせると、意思決定者が動けるタイミングを逸する。特に工数超過・スケジュール遅延は早めに報告するほど打ち手の選択肢が広がる。

> [!TIP]
> **ケイデンスが崩れそうなときこそ早期に宣言する。** 定例報告のタイミングが守れない場合（業務過多・休暇等）は、事前にその旨を関係者に伝えておくことで信頼を保てる。無言で遅延するより、「今週は〇日に送ります」と一言添える方が格段に印象が良い。
...
> [!TIP]
> **議事録は「決定事項」から書く。** 議論の経緯より「何が決まったか」「誰が何をいつまでにやるか」が読まれる。テンプレートの構成も「1. 決定事項 → 2. 議論事項 → 3. ToDo」の順にすると、忙しいステークホルダーが最初のセクションだけ読めば済む設計になる。

> [!WARNING]
> **「全員了承」のまま議事録を共有するのは危険。** 会議で「特に異論なし」となった事項も、誰が承認者かを明記し、内容を確認した旨を返信してもらう運用にしておくと、後から「そんな話は聞いていない」というトラブルを防げる。
...
#### 6.1.3. タスク・課題管理ツール: Redmine (タスク情報)

本プロジェクトでは **Redmine** を進捗・課題管理の中核ツールとして使用します。

* **用途:** タスクチケット（進捗管理トラッカー）と課題チケット（課題トラッカー）の2つのトラッカーでプロジェクトの状態を一元管理します。個別チケットの担当者・期日・ステータスをリアルタイムに把握し、CSVエクスポートによる定例報告資料の作成にも活用します。
* **役割:** タスクの担当者・期限・ステータスの可視化、課題の重要度・影響範囲管理。
* **ルール:**
  - 「誰が」「いつまでに」を明確にし、進捗はRedmineチケット上で更新する
  - タスクチケット（進捗管理トラッカー）: 1チケット = 1担当者 × 1タスク、粒度は1週間が目安（最大3週間）
  - 課題チケット（課題トラッカー）: タスクとは別トラッカーで管理。重要度・優先度・影響範囲・対応方針を記入する
  - 緊急の課題は即日チケット起票し、PMに通知する
* **運用:**
  - 担当者は自身のチケット進捗を日次で更新する
  - PMは週次でRedmineのカスタムクエリを確認し、期限超過・遅延チケットを洗い出す
  - 週次報告時は CSVエクスポート → Excel集計 → スライド反映 のフローで進める

> [!NOTE]
> 汎用ツール（JIRA / Asana / Trello等）に切り替える場合も、「タスクチケット」と「課題チケット」を別管理するという考え方は維持することを推奨します。混在させると課題の見落しが発生しやすくなります。

### 6.2. ツール間の連携ルール

| ツール | 連携先 | 連携内容 | 連携頻度 |
| :--- | :--- | :--- | :--- |
| Slack / Teams Chat | SharePoint | 重要な決定事項、承認 | 随時 |
| Redmine | SharePoint | 課題管理表、進捗報告書 | 週次（CSVエクスポート） |
...
> [!WARNING]
> **「AはPMに決まっているから書かなくていい」は禁物。** 暗黙知でA（説明責任者）を運用すると、プロジェクト後半で「誰がどこまで責任を持っていたか」が曖昧になり、責任の押し付け合いが発生する。最初は面倒でも、全タスクにA/Rを明示的に書き込む習慣をつけると後のトラブルが激減する。

> [!TIP]
> **RACIの作りすぎに注意。** マトリクスを精緻に作り込むほど形骸化しやすい。まず「報告書作成」「エスカレーション」「会議体運営」の3カテゴリだけ作って回し、フェーズが進むにつれて足りない部分を追加する方が実践的。
...
> [!WARNING]
> **「もう少し様子を見てから報告しよう」が最も多い失敗パターン。** エスカレーションの適切なタイミングを逃すと、解決策の選択肢が急速に狭まる。「自分でなんとかできるはず」という判断でレベル1のまま抱え込み、気づいたときにはレベル3相当の問題になっているケースが非常に多い。迷ったら上のレベルに上げてよい。

> [!TIP]
> **Redmineで課題をチケット化してからエスカレーションする。** 口頭だけで報告せず、まずRedmineに課題チケットを起票（重要度・影響範囲・対応方針を記入）した上で会議やチャットで報告する。チケットがあることで後から経緯を追えるし、PMが状況を俯瞰しやすくなる。
...
> [!WARNING]
> **「口頭でOKをもらった」は後から必ず否定される。** 実務でよく発生するのが「あのとき言ったじゃないですか」「そんなこと言っていない」の水掛け論。承認はメール・チャット返信・議事録への記載など、テキストに残る形で必ず記録する。チャット承認であっても「了解しました」の一文があれば証跡になる。
...
> [!WARNING]
> **「最終版」「最新」という言葉は存在してはいけない。** 実際のプロジェクトでは「最終版_修正2」「最終版_20241201」「本当に最終版」のような悲劇的な状況が頻発する。バージョン番号を使えばこれが完全に防げる。チームへの周知とテンプレート化が徹底への近道。
